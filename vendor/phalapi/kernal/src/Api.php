...
```
    ...
    protected function userCheck() {
        $current_url = DI()->request->getServiceApi() . DI()->request->getServiceAction();
        $no_need_token_arr = ['userregister','userlogin','userforgetPwd','topicindex','topicdetail','userhomePage','useruserPublish'];

        if ( !in_array($current_url, $no_need_token_arr) ) {
            // 验证token            
            $token = DI()->request->get('token','bad gay');
